...
• Category: ${certificate.category}` : ""}`,
          ...
          for (const member of params.members) {
            try {
              ...
              await generateSingleCertificate(
                ...
                defaults,
                params.dateFormat,
                layoutConfig, // Pass layout config for score rendering
  ...
  const generateSingleCertificate = async (
    ...
    dateFormat: string,
    layoutConfig: TemplateLayoutConfig | null, // Layout config for score rendering
    scoreData?: Record<string, string> // Optional score data for dual templates
  ) => {
    ...
    // Render score image if dual template and score data provided
    let scoreImageDataUrl: string | undefined;
    if (template.score_image_url && scoreData && layoutConfig?.score?.textLayers) {
      console.log('🎯 Rendering score image for dual template...');
      console.log('📊 Score data:', scoreData);
      
      // CRITICAL: Migrate score layers - ensure all have required properties
      // Same migration logic as certificate (line 465-469)
      const migratedScoreLayers = layoutConfig.score.textLayers.map((layer: TextLayerConfig) => ({
        ...layer,
        maxWidth: layer.maxWidth || 300, // Default maxWidth if missing
        lineHeight: layer.lineHeight || 1.2, // Default lineHeight if missing
      }));
      
      // Prepare score text layers with score data
      const scoreTextLayers: RenderTextLayer[] = migratedScoreLayers.map((layer: TextLayerConfig) => {
        // CRITICAL: Check if layer uses defaultText (like score_date)
        // If useDefaultText is true, use defaultText instead of scoreData
        let text = '';
        if (layer.useDefaultText && layer.defaultText) {
          text = layer.defaultText;
          console.log(`📅 Using defaultText for ${layer.id}:`, text);
        } else if (layer.id === 'issue_date') {
          // CRITICAL: issue_date in score should use certificate issue_date
          text = formatDateString(finalCertData.issue_date, dateFormat);
          console.log(`📅 Using certificate issue_date for score layer:`, text);
        } else {
          text = scoreData[layer.id] || ''; // Get value from scoreData map
        }
      ...
      console.log('📊 Score text layers to render (FULL DATA):', scoreTextLayers.map(l => ({
        id: l.id,
        text: l.text,
        x: l.x,
        y: l.y,
      ...
      // CRITICAL: Verify all layers have valid coordinates
      const invalidLayers = scoreTextLayers.filter(l => 
        (l.x === undefined || l.x === null) && 
        (l.xPercent === undefined || l.xPercent === null)
      );
      if (invalidLayers.length > 0) {
        console.error('❌ INVALID SCORE LAYERS (no x coordinate):', invalidLayers.map(l => l.id));
      }
      
      scoreImageDataUrl = await renderCertificateToDataURL({
      ...
      console.log('✅ Score image rendered successfully');
    }
    ...
    const certificateDataToSave: CreateCertificateData = {
      ...
      score_image_url: scoreImageDataUrl, // Save score image for dual template
      score_data: scoreData || null, // Save score data for dual template
    };
  ...
  async function openPreview(certificate: Certificate) {
    ...
    if (certificate.template_id) {
      try {
        ...
        // CRITICAL: Load score layout from DATABASE (not localStorage) for consistency
        // This ensures score text layers are positioned correctly
        if (template && template.is_dual_template) {
          try {
            const layoutConfig = await getTemplateLayout(certificate.template_id);
            if (layoutConfig?.score?.textLayers) {
              console.log('📊 Loading score layout from database');
              console.log('📊 Score textLayers from DB:', layoutConfig.score.textLayers.map(l => ({
                id: l.id,
                x: l.x,
                y: l.y,
                xPercent: l.xPercent,
              ...
              // Convert database layout to TemplateDefaults format for preview
              const scoreDefaults: TemplateDefaults = {
                templateId: template.id,
                templateName: template.name,
                textLayers: layoutConfig.score.textLayers,
                overlayImages: layoutConfig.score.overlayImages || [],
                savedAt: layoutConfig.lastSavedAt
              };
              setScoreDefaults(scoreDefaults);
              console.log('✅ Score layout loaded from database:', scoreDefaults.textLayers.length, 'layers');
            } else {
              console.warn('⚠️ No score layout in database, trying localStorage fallback');
              const defaults = getTemplateDefaults(`${template.id}_score`);
              setScoreDefaults(defaults);
            }
          } catch (err) {
            console.warn('Failed to load score layout from database, trying localStorage fallback', err);
            try {
              const defaults = getTemplateDefaults(`${template.id}_score`);
              setScoreDefaults(defaults);
            } catch (err2) {
              console.warn('Failed to load score defaults from localStorage', err2);
                  ...
                  >
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 border-2 border-dashed border-blue-200 dark:border-blue-500/40">
                      <div
                        ...
                        className="bg-white dark:bg-gray-950 rounded-xl shadow-xl relative overflow-hidden"
                        style={{
                          width: "100%",
                          maxWidth: "100%",
                          aspectRatio: "800 / 600",
                        ...
                        >
                          <div
                            style={{
                              ...
                              transform: containerDimensions 
                                ? `scale(${Math.min(containerDimensions.width / STANDARD_CANVAS_WIDTH, containerDimensions.height / STANDARD_CANVAS_HEIGHT)})`
                                : 'scale(1)',
                        ...
                        {(() => {
                          ...
                          <>
                            ...
                            {previewMode === 'score' && previewTemplate && previewTemplate.score_image_url ? (
                              <div 
                                className="relative w-full h-full cursor-pointer hover:opacity-95 transition-opacity"
                                onClick={() => setFullScreenImage(previewTemplate.score_image_url || '')}
                                title="Click to view full screen"
                              >
                                <Image
                              ...
                              <div 
                                className="relative w-full h-full cursor-pointer hover:opacity-95 transition-opacity"
                                onClick={() => setFullScreenImage(getTemplateImageUrl(previewTemplate) || '')}
                                title="Click to view full screen"
                              >
                                <Image
                            ...
                            {/* CANVA-STYLE: Text Layers with FIXED document size (no per-layer scaling) */}
                            {previewMode === 'certificate' && previewCertificate.text_layers &&
                              previewCertificate.text_layers.map(
                                (layer: CertificateTextLayer) => {
                                  // Use absolute pixel coordinates (document space)
                                  // Fallback to percentage if absolute not available (backward compat)
                                  const actualX = layer.x !== undefined 
                                    ? `${layer.x}px` 
                                    : `${(layer.xPercent || 0) * STANDARD_CANVAS_WIDTH}px`;
                                  const actualY = layer.y !== undefined 
                                    ? `${layer.y}px` 
                                    : `${(layer.yPercent || 0) * STANDARD_CANVAS_HEIGHT}px`;
                                  
                                  // CANVA-STYLE: Use ORIGINAL fontSize (not scaled)
                                  // Scaling is handled by parent container transform
                                  const fontSize = layer.fontSize;
                                  
                                  // CRITICAL: certificate_no and issue_date NEVER wrap (always single line)
                                  const isSingleLineField = layer.id === 'certificate_no' || layer.id === 'issue_date';
                                  const maxWidth = isSingleLineField ? undefined : layer.maxWidth;

                                  // Match configure page transform behavior based on textAlign
                                  // certificate_no and issue_date always use left alignment
                                  const textAlign = isSingleLineField ? 'left' : (layer.textAlign || 'left');
                                  const getTransform = () => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        fontFamily: layer.fontFamily,
                                        textAlign: textAlign,
                                        whiteSpace: isSingleLineField ? 'nowrap' : (maxWidth ? 'normal' : 'nowrap'),
                                        width: maxWidth ? `${maxWidth}px` : 'auto',
                                        lineHeight: layer.lineHeight || 1.2,
                                        wordWrap: maxWidth ? 'break-word' : 'normal',
                                        overflowWrap: maxWidth ? 'break-word' : 'normal',
                                        userSelect: "none",
                                        pointerEvents: "none",
                            ...
                            {previewMode === 'score' && scoreDefaults && scoreDefaults.textLayers && scoreDefaults.textLayers.map((layer: TextLayerDefault) => {
                              // CRITICAL: Match rendering logic - prioritize absolute coordinates
                              // Same as certificate-render.ts line 125-130
                              const actualX = layer.x !== undefined 
                                ? `${layer.x}px` 
                                : `${(layer.xPercent || 0) * STANDARD_CANVAS_WIDTH}px`;
                              const actualY = layer.y !== undefined 
                              ...
                              if (layer.id === 'score_date' || !layer.x) {
                                console.log(`🔍 Score layer [${layer.id}] position:`, {
                                  x: layer.x,
                                  y: layer.y,
                                  xPercent: layer.xPercent,
                                  yPercent: layer.yPercent,
                              ...
                              // CRITICAL: Use actual score_data from certificate (same as rendering logic)
                              let content = '';
                              if (layer.useDefaultText && layer.defaultText) {
                                // Use defaultText for layers like score_date
                                content = layer.defaultText;
                              } else if (previewCertificate.score_data && typeof previewCertificate.score_data === 'object') {
                                // Use actual score data from certificate
                                content = (previewCertificate.score_data as Record<string, string>)[layer.id] || '';
                              } else {
                                // Fallback to layer ID
                                content = layer.id.replace(/_/g, ' ');
                              }

                              // CANVA-STYLE: Use ORIGINAL fontSize (not scaled)
                              const fontSize = layer.fontSize;
                              const maxWidth = layer.maxWidth;

                              // Match configure page transform behavior
