...
export interface RenderTextLayer {
  ...
  maxWidth?: number;
  lineHeight?: number;
  // Rich text support for inline formatting
  richText?: RichText;
...
export async function renderCertificateToDataURL(
  ...
  // CRITICAL: Explicitly load and validate fonts used in text layers
  // document.fonts.ready doesn't guarantee fonts are available in Canvas context
  const uniqueFonts = new Set(textLayers.map(l => l.fontFamily || 'Arial'));
  console.log('🔤 Fonts to load:', Array.from(uniqueFonts));
  
  for (const fontFamily of uniqueFonts) {
    try {
      // Try to load font explicitly
      await document.fonts.load(`16px ${fontFamily}`);
      
      // Verify font is actually loaded
      const fontAvailable = document.fonts.check(`16px ${fontFamily}`);
      if (!fontAvailable) {
        console.warn(`⚠️ Font "${fontFamily}" not available, will fallback to Arial`);
      } else {
        console.log(`✅ Font "${fontFamily}" loaded and verified`);
      }
    } catch (err) {
  ...
  for (const layer of textLayers) {
    ...
    // CRITICAL: Prioritize absolute coordinates (x, y) over percentage
    // This matches preview and configure page behavior
    // Fallback to percentage only if absolute coordinates are not available
    ...
    // CRITICAL: Verify font is available, fallback to Arial if not
    const fontAvailable = document.fonts.check(`${scaledFontSize}px ${fontFamily}`);
    if (!fontAvailable && fontFamily !== 'Arial') {
      console.warn(`⚠️ [${layer.id}] Font "${fontFamily}" not available in Canvas, using Arial fallback`);
      fontFamily = 'Arial';
    }
    
    ctx.font = `${fontWeight} ${scaledFontSize}px ${fontFamily}`;
    
    // CRITICAL: Test if font is actually working by measuring a test string
    const testMetrics = ctx.measureText('Test');
    if (testMetrics.actualBoundingBoxAscent <= 0 || isNaN(testMetrics.actualBoundingBoxAscent)) {
      console.error(`🚨 [${layer.id}] Font "${fontFamily}" produces invalid metrics, forcing Arial`);
      fontFamily = 'Arial';
    ...
    // Check if layer has rich text (inline formatting)
    if (layer.richText && layer.hasInlineFormatting) {
      console.log(`🎨 [${layer.id}] Rendering with RICH TEXT (inline formatting)`);
      drawRichText(
        ctx,
        layer.richText,
        x,
      ...
      // Render as plain text (backward compatible)
      drawWrappedText(
        ctx, 
        layer.text, 
        x, 
...
function drawWrappedText(
  ...
  // - certificate_no & issue_date (small fonts): Use visual center calculation with hardcoded adjustments
  // - Score layers (small fonts): Use geometric center of line-height box (no hardcoded adjustments)
  //
  const isCertificateLayer = layerId === 'certificate_no' || layerId === 'issue_date';
  
  // Detect score layers: Not certificate layers (name, certificate_no, issue_date) AND small font
  // Score layers typically have fontSize < 20px and are not certificate-specific fields
  const isScoreLayer = layerId !== 'name' && 
  ...
  if (isCertificateLayer) {
    ...
    // For score layers: Use geometric center of line-height box (like certificate_no but without hardcoded adjustments)
    // This provides better centering for small fonts compared to simple geometric center
    const geometricCenterOfLineBox = lineHeightPx / 2;
    const visualCenterOfBlock = geometricCenterOfLineBox + ((lines.length - 1) * lineHeightPx) / 2;
    
    // No hardcoded adjustment - let CSS and Canvas naturally align
    // This should work better than simple geometric center for small fonts
    startY = y - visualCenterOfBlock;
    
    console.log(`✨ [${layerId}] Using SCORE LAYER centering (geometric center of line-height box):`, {
      layerId,
      fontSize,
      lineHeight,
      lineHeightPx,
      geometricCenterOfLineBox,
      visualCenterOfBlock,
      startY,
      y,
      method: 'GEOMETRIC_CENTER_LINEBOX - same as certificate_no but without adjustments'
    });
  } else {
  ...
  if (layerId === 'name' || layerId === 'certificate_no' || layerId === 'issue_date') {
    ...
    console.log(`🔍 [${layerId}] DRAW_WRAPPED_TEXT CALCULATION:`, {
      ...
      calculationMethod: isCertificateLayer 
        ? (layerId === 'issue_date'
      ...
      // For certificate layers: Show calculation details
      ...(isCertificateLayer ? {
        ...(layerId === 'issue_date' ? {
          // issue_date uses geometric center with padding/border adjustment
...
/**
 * Draw rich text with inline formatting (per-word/character styles)
 * Similar to drawWrappedText but handles multiple text spans with different styles
 */
function drawRichText(
  ...
  // Convert rich text spans to plain text for wrapping calculation
  // const fullText = richText.map(span => span.text).join('');
  // Reserved for Phase 2: Advanced wrapping with mixed styles
  
  // Build array of styled segments with their positions
  ...
  const segments: StyledSegment[] = richText.map(span => ({
    text: span.text,
    fontWeight: String(span.fontWeight || baseFontWeight),
    fontFamily: span.fontFamily || baseFontFamily,
    fontSize: (span.fontSize || baseFontSize) * scaleFactor,
    color: span.color || baseColor
  }));
  
  // For simplicity in Phase 1: render as single line with inline styles
  // Advanced wrapping with mixed styles is complex - defer to Phase 2
  ctx.textBaseline = 'top';
  ctx.textAlign = textAlign === 'center' ? 'center' : (textAlign === 'right' ? 'right' : 'left');
  
  // Calculate total text height for vertical centering
  const maxSegmentFontSize = Math.max(...segments.map(s => s.fontSize));
  const lineHeightPx = maxSegmentFontSize * lineHeight;
  ...
  let drawX = x;
  if (textAlign === 'center') {
    // For center alignment, calculate total width and center it
    let totalWidth = 0;
    segments.forEach(seg => {
      const weight = seg.fontWeight === 'bold' ? 'bold' : 'normal';
      ctx.font = `${weight} ${seg.fontSize}px ${seg.fontFamily}`;
      totalWidth += ctx.measureText(seg.text).width;
    });
    drawX = x - (totalWidth / 2);
  } else if (textAlign === 'right') {
    // For right alignment, calculate total width and align right
    let totalWidth = 0;
    segments.forEach(seg => {
      const weight = seg.fontWeight === 'bold' ? 'bold' : 'normal';
      ctx.font = `${weight} ${seg.fontSize}px ${seg.fontFamily}`;
  ...
  segments.forEach((seg, index) => {
    ...
    const segmentWidth = ctx.measureText(seg.text).width;
    currentX += segmentWidth;
    
    if (index === 0 && layerId) {
      console.log(`🎨 [${layerId}] RICH TEXT RENDER:`, {
        layerId,
        segmentText: seg.text.substring(0, 20),
        fontWeight: seg.fontWeight,
        fontFamily: seg.fontFamily,
        fontSize: seg.fontSize,
        color: seg.color,
        drawX: currentX - segmentWidth,
        startY,
        textAlign
      });
    }
