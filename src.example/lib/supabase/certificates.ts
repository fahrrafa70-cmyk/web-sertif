...
/**
 * Score Data for Dual Template
 * Flexible structure untuk menyimpan berbagai jenis nilai
 */
export interface ScoreData {
  // Nilai-nilai umum
  theory_score?: number | string;      // Nilai teori
  practice_score?: number | string;    // Nilai praktek
  attitude_score?: number | string;    // Nilai sikap/attitude
  total_score?: number | string;       // Total nilai
  grade?: string;                      // Grade (A, B, C, dll)
  
  // Kompetensi (bisa ada multiple)
  competencies?: {
    name: string;
    score: number | string;
    description?: string;
  ...
  // Custom fields (untuk flexibility)
  // User bisa tambahkan field apapun sesuai kebutuhan
  [key: string]: unknown;
}
...
export interface Certificate {
  ...
  score_data: ScoreData | null; // NEW: Score data untuk dual template
  created_at: string;
  updated_at: string;
...
export interface CreateCertificateData {
  ...
  text_layers?: TextLayer[];
  score_data?: ScoreData | null; // NEW: Score data for dual template
