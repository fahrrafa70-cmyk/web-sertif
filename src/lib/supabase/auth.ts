...
export async function checkEmailWhitelist(
  email: string,
): Promise<"owner" | "manager" | "staff" | "user" | null> {
  try {
...
export async function createOrUpdateUserFromOAuth(
  ...
  try {
    ...
    if (whitelistRole) {
      ...
      console.log("🔍 [OAUTH] Using whitelist role:", whitelistRole);
      ...
      if (
        ...
        console.log("🔍 [OAUTH] Preserving existing user role:", currentRole);
      } else if (currentRole === "admin") {
        roleToPersist = "owner";
        console.log("🔍 [OAUTH] Mapping admin to owner");
      } else if (currentRole === "team" || currentRole === "member") {
        roleToPersist = "manager";
        console.log("🔍 [OAUTH] Mapping team/member to manager");
      } else if (currentRole === "public") {
        roleToPersist = "user";
        console.log("🔍 [OAUTH] Mapping public to user");
      }
    }
    ...
    console.log(
      `✅ User upserted via API: ${normalizedEmail} role=${roleToPersist}`,
      result?.data,
    );
  } catch (err) {
...
export async function getUserRoleAndSubscription(email: string): Promise<{
  ...
  try {
    ...
    if (!error && data) {
      ...
      console.log("🔍 [AUTH] Final result from whitelist:", {
        role: mappedRole,
        hasSubscription,
      });
    ...
    console.error(
      "Error checking whitelist in getUserRoleAndSubscription:",
      err,
    );
  }
