...
export interface Template {
  ...
  use_check_number?: boolean; // Whether this template uses sequential check_number for /cek/ URLs (001, 002, etc.)
}
...
export async function uploadTemplateImage(file: File): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Upload to storage failed: ${response.status} ${errorText}`,
    );
  }
...
export async function createTemplate(
  ...
  try {
    ...
    if (
      ...
      !templateData.orientation?.trim()
    ) {
      throw new Error(
        "Missing required fields: name, category, and orientation are required",
      );
    }
    ...
    if (templateData.is_dual_template) {
      ...
      if (templateData.certificate_image_file) {
        try {
          const uploadResult = await uploadTemplateImage(
            templateData.certificate_image_file,
          );
          certificateImagePath = uploadResult;
          certificateThumbnailPath = uploadResult; // Use same URL for thumbnail
        } catch (uploadError) {
          throw new Error(
            `Certificate image upload failed: ${uploadError instanceof Error ? uploadError.message : "Unknown error"}`,
          );
        }
      } else {
      ...
      if (templateData.score_image_file) {
        try {
          const uploadResult = await uploadTemplateImage(
            templateData.score_image_file,
          );
          scoreImagePath = uploadResult;
          scoreThumbnailPath = uploadResult; // Use same URL for thumbnail
        } catch (uploadError) {
          throw new Error(
            `Score image upload failed: ${uploadError instanceof Error ? uploadError.message : "Unknown error"}`,
          );
        }
      } else {
      ...
      if (templateData.preview_image_file) {
        try {
          const uploadResult = await uploadTemplateImage(
            templateData.preview_image_file,
          );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error || errorData.details || "Failed to create template",
      );
    }
...
export async function updateTemplate(
  ...
  try {
    ...
    let previewImagePath: string | undefined =
      currentTemplate.preview_image_path;
    let certificateImagePath: string | undefined =
      currentTemplate.certificate_image_url;
    let scoreImagePath: string | undefined = currentTemplate.score_image_url;

    // Handle dual template mode
    if (templateData.is_dual_template) {
      console.log(
        "🔄 Dual template mode - handling certificate and score images...",
      );

      // Handle certificate image update
      if (templateData.certificate_image_file) {
        console.log(
          "📤 New certificate image file provided, starting upload...",
        );
        try {
          const uploadResult = await uploadTemplateImage(
            templateData.certificate_image_file,
          ...
          imagePath = uploadResult;
          console.log(
            "✅ Certificate image upload completed, path:",
    ...
    if (templateData.preview_image_file) {
      ...
      try {
        ...
        previewImagePath = uploadResult;
        console.log(
          "✅ Preview image upload completed, path:",
          previewImagePath,
        );
      } catch (e) {
    ...
    if (templateData.status !== undefined && templateData.status !== null) {
      if (templateData.status === "ready" || templateData.status === "draft") {
        ...
        console.log(
          "✅ Status included in update:",
          templateData.status,
          "type:",
          typeof templateData.status,
        );
      } else {
    ...
    console.log(
      "💾 Updating template data via API:",
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error || errorData.details || "Failed to update template",
      );
    }
    ...
    if (updateData.status && !data.status) {
      console.warn(
        "⚠️ Status was in updateData but missing from API response, adding it manually",
      );
    ...
    if (verifyError) {
      ...
      console.warn(
        "⚠️ Verification failed, returning API response data with status from updateData",
      );
      return data;
    }
    ...
    if (!verifyData?.status && updateData.status) {
      console.warn(
        "⚠️ Status not in verified data, using from updateData:",
        updateData.status,
      );
...
export async function deleteTemplate(id: string): Promise<void> {
  ...
  try {
    ...
    if (template.image_path) {
      try {
        console.log(
          "🖼️ Deleting associated image file...",
          template.image_path,
        );
        ...
        if (fileName) {
          ...
          console.warn(
            "⚠️ Could not extract filename from image path:",
            template.image_path,
          );
        }
      } catch (imageError) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error || "Failed to delete template from database",
      );
    }
    ...
    if (verifyError) {
      ...
      console.error(
        "❌ Template still exists in database after delete!",
        verifyData,
      );
      throw new Error(
        "Template deletion failed - template still exists in database",
      );
    } else {
...
export function validateLayoutConfig(
  ...
  const existingLayerIds =
    config.certificate.textLayers?.map((l) => l.id) || [];
  ...
  try {
    ...
    if (!template.is_layout_configured) {
      return {
        ready: false,
        message:
          "Template layout is not configured. Please configure it in Templates page first.",
      };
    }
