...
ΓÇó Certificate Number: ${certificate.certificate_no || "N/A"}
ΓÇó Recipient Name: ${certificate.name || "N/A"}
ΓÇó Issue Date: ${new Date(certificate.issue_date || certificate.created_at || new Date()).toLocaleDateString()}${certificate.expired_date ? `
ΓÇó Expiry Date: ${new Date(certificate.expired_date).toLocaleDateString()}` : ""}${certificate.category ? `
ΓÇó Category: ${certificate.category}` : ""}`;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="mt-4 space-y-1 text-xs sm:text-sm">
                <div><span className="text-gray-500 dark:text-gray-400">{t('hero.category')}:</span> {previewCert!.category || "ΓÇö"}</div>
                <div><span className="text-gray-500 dark:text-gray-400">{t('hero.template')}:</span> {(previewCert as unknown as { templates?: { name?: string } }).templates?.name || "ΓÇö"}</div>
